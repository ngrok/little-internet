...
Wireshark knows the ethertype and then gives up on everything after it:

```
Ethernet II, Src: 3c:78:95:3e:f4:62, Dst: ff:ff:ff:ff:ff:ff
    Type: Realtek Layer 2 Protocols (0x8899)
Realtek Layer 2 Protocols
    Unknown packet: 250000003c78953ef46200000000003c78953ef462ff004d8a1eef...
```

`0x8899` is Realtek's private sandbox. Wireshark can decode three things inside
it (a remote control protocol, an echo protocol, and a loop detection protocol),
and this frame is none of them. The dissector picks the right one by reading
the first payload byte, `0x25` here, which isn't in its table. So it shrugs
and hands me 46 undecoded bytes.

I can read part of it without help. `3c78953ef462` is the switch's own MAC
address, and it appears in there twice. The frame is the switch saying its own
name, to everybody, once a second, forever.

The rest of it looked like noise until I put the two captures next to each
other. Every one of these frames is identical, byte for byte, except in one
position:

```
pi-foo-01:  ...3c78953ef462 ff 00 4d8a1eef...
pi-foo-02:  ...3c78953ef462 ff 01 4d8a1eef...
```

`00` and `01`. That's the port number.

(Once the third Pi showed up later in this diary, I checked it too, and got
`02`. Three ports, three numbers.)

Which finally tells me what the frame is _for_. The switch sends this out of
every port and stamps each copy with the port it just left by. If the copy that
went out port 1 ever comes back in on port 3, then ports 1 and 3 are wired into
a loop, and the switch can prove it, because its own name is on the frame with a
port number attached to it.

So it is loop detection, and the port byte is the entire trick. TP-Link calls it
"Loop Prevention" in the switch's web UI, and switching it off stops the frames.
I'm leaving it on. It costs one 60-byte frame per second, and it's the only
thing on this network with an opinion about its own topology.
...
all. The client never made the suggestion, so I never gave `dnsmasq` anything to
respect.
...
3. BOM.md phase 1 lists two Pis and three SD cards. This episode adds a third
   Pi, an OLED, a case, and a PSU.
4. lessons/02/ has captures but no README.md or manifest.json yet. Lesson 01 has
   both, and the captures should get linked the way lesson 01 links its own.
